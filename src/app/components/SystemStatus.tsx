...
export default function SystemStatus({
    ...
    const getOverallStatus = () => {
        ...
        if (
            ...
            activeStatuses.length < activeProjects.length
        ) {
            return {
                loading: true,
        ...
        const offlineCount = activeStatuses.filter(
            (status) => status === "offline"
        ).length;
        const loadingCount = activeStatuses.filter(
        ...
        if (offlineCount === 0 && loadingCount === 0) {
            ...
            return {
                loading: false,
                isOperational: false,
                message: `${unavailableCount} System${
    ...
    const checkProjectStatus = async (project: Project) => {
        ...
        try {
            const res = await fetch(
                `/api/ping?url=${encodeURIComponent(project.url)}`
            );
        ...
        <>
            ...
            <div className="text-center">
                ...
                {overallStatus.loading ? (
                    <div
                        className={`flex items-center justify-center gap-2 my-4 ${overallStatus.color}`}
                    >
                        <div className="relative w-2 h-2 mr-1 flex items-center justify-center">
                            <div
                                className={`absolute inset-0 ${overallStatus.dotColor} rounded-full animate-pulse opacity-75`}
                            ></div>
                            <div
                                className={`absolute inset-0 ${overallStatus.dotColor} rounded-full animate-ping opacity-75`}
                            ></div>
                        </div>
